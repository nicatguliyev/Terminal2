            ...
            public void onClick(View view) {
                ...
                for(int i = 0; i<3; i++){
                    if(spinners[i].getSelectedItemPosition() == 0){
                        Toast.makeText(getApplicationContext(), messages[i] + " seçilməyib", Toast.LENGTH_SHORT).show();
                        count = 1;
                        break;
                    }
                }
            ...
            public void onResponse(String response) {
                ...
                try {
                    Log.i("VAqonlar",  response);
                      ...
                      String result = jsonObject.getString("result");
                      if(result.equals("0")){
                          Toast.makeText(getApplicationContext(), "Vaqon Yoxdur", Toast.LENGTH_SHORT).show();
                      }
                      else
                      {
